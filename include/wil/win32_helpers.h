...
namespace details
{
    ...
    struct process_entry
    {
        ...
        process_entry(std::wstring_view processName = L"") :
            m_processName(!processName.empty() ? ::wil::make_unique_string<::wil::unique_cotaskmem_string>(processName.data()) : nullptr),
    ...
    private:
        ::wil::shared_cotaskmem_string m_processName;
        ::wil::shared_handle m_hSnapshot;

        bool matches_name(std::wstring_view processName) const
        {
            return !m_processName || (_wcsicmp(processName.data(), m_processName.get()) == 0);
        }
...
{
    explicit process_iterator(std::wstring_view processName = L"") : m_processEntry(processName)
    {
    }
